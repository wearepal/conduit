...
class MNIST(VisionDataset):
    ...
    def __getitem__(self, index: int) -> Tuple[Any, Any]:
        ...
        # TODO: use opencv to load images instead
    ...
    _FILE_LIST: Final = [
        (
            "1g61HbYkoDoh4s4BuEBsOMwGp7IZ5miAG",  # File ID
            "f68b3c2dcbeaaa9fbdd348bbdeb94873",  # MD5 Hash
            "train-images-idx3-ubyte.gz",  # Filename
        ),
        (
            "17347x3HV7OoUPiCZ7NsbEB-MLY1lMDLV",  # File ID
            "d53e105ee54ea40749a09fcbcd1e9432",  # MD5 Hash
            "train-labels-idx1-ubyte.gz",  # Filename
        ),
        (
            "1NDTc1Z3E0OBbq_-bhvKp1KUjjFYP0CR8",  # File ID
            "9fb629c4189551a2d022fa330f9573f3",  # MD5 Hash
            "t10k-images-idx3-ubyte.gz",  # Filename
        ),
        (
            "17iUutenIILIPIqzVRFGfxt9bCezzWWzQ",  # File ID
            "ec29112dd5afa0611ce80d1b7f02629c",  # MD5 Hash
            "t10k-labels-idx1-ubyte.gz",  # Filename
        ),
    ]
    ...
    def download(self) -> None:
        ...
        for id, md5, filename in self._FILE_LIST:
            download_file_from_google_drive(id, self.raw_folder, filename, md5)
            archive = os.path.join(self.raw_folder, filename)
            print("Extracting {} to {}".format(archive, self.raw_folder))
            extract_archive(archive, self.raw_folder, False)
...
class EMNIST(MNIST):
    ...
    def download(self) -> None:
        ...
        for split in self.splits:
            ...
            training_set = (
                read_image_file(
                    os.path.join(gzip_folder, 'emnist-{}-train-images-idx3-ubyte'.format(split))
                ),
                read_label_file(
                    os.path.join(gzip_folder, 'emnist-{}-train-labels-idx1-ubyte'.format(split))
                ),
            )
            test_set = (
                read_image_file(
                    os.path.join(gzip_folder, 'emnist-{}-test-images-idx3-ubyte'.format(split))
                ),
                read_label_file(
                    os.path.join(gzip_folder, 'emnist-{}-test-labels-idx1-ubyte'.format(split))
                ),
            )
